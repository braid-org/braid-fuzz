...
class ClientBridge extends EventEmitter {
    constructor({ command, args = [], cwd, env, server_base_url }) {
        ...
        this.alive = false
        this.stderr_lines = []
        this.updates = []         // update events pushed by the client
        this.errors = []          // error events pushed by the client
        this.acks = []            // ack events (PUT responses)
        this.child_added = []     // {url, name} — webindex watch events
        this.child_removed = []   // {url, name} — webindex watch events
    ...
    _handle_response(msg) {
        if (msg.id && this._pending.has(msg.id)) {
            ...
            if (msg.event === "update") this.updates.push(msg.data)
            if (msg.event === "error") this.errors.push(msg.data)
            if (msg.event === "ack") this.acks.push(msg.data)
            if (msg.event === "child-added") this.child_added.push(msg.data)
            if (msg.event === "child-removed") this.child_removed.push(msg.data)
            this.emit("event", msg)
        }
    }
    ...
    async connect(url, opts = {}) {
        ...
        return this.send("sync-text", { url: full_url, ...opts })
    }
    async insert(pos, text) { return this.send("edit", { pos, len: 0, text }) }
    async delete(pos, len) { return this.send("edit", { pos, len, text: "" }) }
    async replace(pos, len, text) { return this.send("edit", { pos, len, text }) }
    async state() { return (await this.send("send-text")).state }
    async wait_ack() { return this.send("ack") }
    async kill_sub() { return this.send("end-sync") }
    ...
    async stop() {
        ...
        try { await this.send("results").catch(() => {}) } catch (e) {}
